    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="requestObituaryCreation" class="contrast-primary-text" small color="primary" :disabled="!hasRole('manager')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>부고장생성
                </v-btn>
            </div>
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">FuneralInfo 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">FuneralInfo 수정</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
...
export default {
    ...
    methods:{
        async requestObituaryCreation(){
            try{
                var path = "requestObituaryCreation".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, null)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','requestObituaryCreation 성공적으로 처리되었습니다.')
